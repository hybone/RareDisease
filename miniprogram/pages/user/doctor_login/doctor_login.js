...
Page({
  ...
  /*点击提交按钮*/
  onSubmit(res) {
    var {name, hospital, rank, good_at, introduction} = res.detail.value
  ...
  check(res) {
    let {name, hospital, rank, good_at, introduction} = res.detail.value
    if(name=="") {
    ...
    if(introduction=="") {
      Notify({ type: 'danger', duration:1000, message: '请选择个人介绍'})
      return false
    }
